...
- Executed chunk: added a shared route-readiness helper plus a new `Launch truth` section in the `Runs` tab.
- Outcome: dry-run versus live-ready versus blocked launch state is now visible per target before a post is pushed through delivery.
...
## Durable Handoff
- Outcome: execution visibility is materially deeper; the app now states route readiness explicitly instead of making the operator infer it.
- Risk: live connector work is still externally blocked, so this pass intentionally improved launch truth rather than pretending those connectors are complete.
